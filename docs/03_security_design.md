...
| Alert reliability | Ingest Lambda, SQS Incident Queue, SQS DLQ, retry visibility | Worker processing logic sau khi consume alert |
...
| EKS namespace `aiops` | AIOps backend/worker | Được poll SQS, query observability bounded, gọi AI Engine, update DynamoDB/S3. |
...
graph TB
    ...
    subgraph AWS["AWS Cloud"]
        ...
        subgraph VPC["VPC"]
            ...
            subgraph EKS["EKS Cluster - Private Subnets"]
                ...
                AIOPS["Namespace: aiops<br/>AIOps Backend / Worker"]
    ...
    OBS -->|"Alertmanager Webhook"| LAMBDA
    LAMBDA --> SQS
    SQS --> DLQ

    AIOPS -->|"Poll / Consume Incident"| SQS
    AIOPS -->|"Bounded Query"| OBS
    AIOPS -->|"Internal Service Call"| AI
    ...
    CW -.->|"Logs / Metrics / Alarms"| LAMBDA
    CW -.->|"Queue / DLQ / Worker / AI / DDB / Integration Metrics"| AIOPS
```

![mermaid sec](../docs/assets/security-mermaid-diagram.png)
...
- AIOps Worker gọi AI Engine qua internal service DNS, không đi public internet.
...
| AIOps Worker -> SQS | Có | IRSA role chỉ receive/delete/change visibility trên incident queue. |
| AIOps Worker -> Observability | Có | Query bounded theo tenant/service/env/window. |
| AIOps Worker -> AI Engine | Có | Internal service + auth header/service token. |
| AIOps Worker -> Jira/Slack | Có | Secret riêng, retry và idempotency. |
...
### 3.4 Xử lý dữ liệu cho Bedrock / AI
...
- Internal service call giữa AIOps Worker và AI Engine tối thiểu dùng internal service + auth token/JWT.
...
| AIOps Worker | Max concurrency, per-tenant quota, backoff | Không để noisy tenant làm nghẽn tenant khác. |
...
| `tf1-aiops-worker-irsa-role` | K8s service account `aiops/worker` | SQS receive/delete/change visibility, DynamoDB read/write scoped table, S3 put/get scoped prefix, Secrets read scoped ARNs | `AdministratorAccess`, broad `s3:*`, broad `dynamodb:*` |
...
- `aiops-worker-sa` có scoped SQS/DynamoDB/S3/Secrets permissions.
...
| `aiops-worker` | `aiops` | `get`, `list`, `watch` | pods, services, endpoints | Worker không cần cluster-admin. |
...
| `SERVICE_AUTH_TOKEN` | Secrets Manager / External Secrets | AIOps Worker + AI Engine | Manual cho capstone |
| `JIRA_API_TOKEN` | Secrets Manager | AIOps Worker / integration layer | Manual, rotate sau demo nếu dùng token thật |
| `SLACK_WEBHOOK_URL` | Secrets Manager | AIOps Worker / integration layer | Manual, rotate sau demo nếu dùng webhook thật |
...
| Observability query | Bắt buộc có `tenant_id + service + env + time_window`. |
| AIOps Worker | Reject nếu thiếu `tenant_id`, `service`, `env`, `window`. |
...
```text
Alertmanager -> Ingest Lambda -> SQS Incident Queue -> AIOps Worker -> RCA -> Jira/Slack
```
...
### 10.3 Control idempotency bằng DynamoDB
...
| AIOps bounded query | `tenant_id`, `service`, `env`, `window`, query source, result count |
| AI Engine call | `tenant_id`, `correlation_id`, `incident_id`, latency, status, confidence |
...
- AIOps Worker errors, processing latency, throughput.
...
| AIOps Backend / Worker namespace `aiops` | Worker có quyền quá rộng, duplicate Jira/Slack | IRSA least privilege, idempotency check, NetworkPolicy | Per-tenant worker quota, retry with backoff, no cluster-admin, no direct broad observability admin. |
| TF1 AI Engine API namespace `ai-engine` | Public abuse, prompt/log injection, oversized context | Internal service only, auth header, request size limit, bounded context | mTLS/service mesh hoặc JWT service-to-service, no auto-remediation, model guardrails. |
...
| SQ-03 | Final auth giữa AIOps Worker và AI Engine là bearer token, mTLS, hay service-to-service JWT? | CDO-05 + AIO-01 | Before integration |
...
- [`02_infra_design.md`](02_infra_design.md) - source of truth cho architecture diagram, component table, scaling và failure modes.
