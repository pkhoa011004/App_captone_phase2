...
METRICS_FILE = r"apps/simulator/fake-data/evidence/metrics/scenario-1-db-pool-exhaustion.json"
LOGS_FILE = r"apps/simulator/fake-data/evidence/logs/scenario-1-db-pool-exhaustion.json"
TRACES_FILE = r"apps/simulator/fake-data/evidence/traces/scenario-1-db-pool-exhaustion.json"

TARGET_URL = "http://localhost:8080/dumbproxy/inject-batch"

async def main():
    print("=== SCENARIO 1: DATABASE CONNECTION POOL EXHAUSTION TEST ===")
    ...
    # Group logs by timestamp (or match closest timestamp)
    ...
    for trace in traces_data:
        ts = trace["ts"]
        # Match trace ts to timeline points
        # If not exact match, add it to the closest point or create new
        matched_ts = None
    ...
    print("\n[OK] Scenario 1 Telemetry Injection Completed Successfully!")
