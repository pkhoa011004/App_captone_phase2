...
*   **Nội dung**: Gom nhóm 3 alert xảy ra liên tiếp trong khung thời gian 10 phút (từ `10:00:00Z` tới `10:04:00Z` ngày `2026-06-29`):
    1.  `alert-book-service-memory-high` (started_at: `10:00:00Z`): Cảnh báo RAM vượt ngưỡng 90%.
    2.  `alert-book-service-oomkilled` (started_at: `10:02:00Z`): Cảnh báo container bị tiêu diệt do hết bộ nhớ.
    3.  `alert-book-service-crashloop` (started_at: `10:04:00Z`): Cảnh báo Pod rơi vào trạng thái lặp lại lỗi.
...
*   **Dữ liệu RAM (`memory_usage_mb`)**:
    *   Từ `09:45:00Z` tới `09:58:00Z`: Tăng từ 410MB lên 980MB.
    *   Lúc `10:00:00Z` - `10:01:00Z`: Đạt đỉnh `1024.0` (giới hạn cứng của container).
    *   Lúc `10:02:00Z`: Tụt về `200.0` (thời điểm container bị kill và bắt đầu tái khởi động).
...
*   Ghi nhận 3 sự kiện chính trong vòng đời Pod:
    *   `OOMKilled` (10:01:45Z): Cảnh báo `Warning` báo hiệu container bị tắt khẩn cấp với exit code 137.
    *   `BackOff` (10:03:15Z): Kubernetes tạm hoãn việc tái khởi động container.
    *   `CrashLoopBackOff` (10:04:10Z): Trạng thái Pod lỗi liên tục.
...
*   Mô tả quá trình thread tăng đột biến và lỗi cấp phát bộ nhớ native thread:
    *   `09:55:00Z` - Cảnh báo Warning: `"High thread count detected: 1800/2000 limits"`.
    *   `09:58:00Z` - Lỗi Error: `"Thread pool exhausted. Failed to create new native thread. Active threads: 2000"`.
    *   `10:00:00Z` - Lỗi Error: `"java.lang.OutOfMemoryError: unable to create new native thread"`.
    *   `10:01:30Z` - Ghi nhận tín hiệu tắt: `"Container received SIGKILL signal from Kubernetes"`.
...
*   **Nội dung**: Gom nhóm 2 alert xảy ra liên tiếp trong khung thời gian 10 phút (từ `10:00:00Z` tới `10:02:00Z` ngày `2026-06-29`):
    1.  `alert-book-service-cpu-high` (started_at: `10:00:00Z`): Cảnh báo CPU sử dụng vượt ngưỡng 95%.
    2.  `alert-book-service-cpu-throttled` (started_at: `10:02:00Z`): Cảnh báo CPU bị bóp băng thông xử lý > 50%.

#### B. Chỉ số tài nguyên (Metrics Evidence)
*   **Đường dẫn**: `evidence/metrics/scenario_06_cpu_throttling_metrics.json`
*   **Dữ liệu CPU (`cpu_usage_percent`)**: Đạt ngưỡng `100.0` liên tiếp từ `10:00:00Z` đến `10:02:00Z`.
*   **Dữ liệu CPU Throttling (`cpu_throttling_percent`)**: Tăng vọt từ `0.0` lên `65.0` (`10:00:00Z`), đạt đỉnh `75.0` (`10:02:00Z`) trước khi giảm dần khi tải hạ nhiệt.
...
*   Do CPU bị throttle nặng khiến tiến trình không kịp xử lý phản hồi kiểm tra sức khỏe của kubelet:
    *   `Unhealthy` (09:59:30Z): Liveness probe thất bại (kéo dài quá 5000ms).
    *   `Unhealthy` (10:00:45Z): Readiness probe thất bại (kéo dài quá 5000ms).

#### D. Nhật ký hệ thống (Logs Evidence)
*   **Đường dẫn**: `evidence/logs/scenario_06_cpu_throttling_logs.json`
*   Mô tả hiện tượng nghẽn luồng do CPU bị kiềm chế:
    *   `09:55:00Z` - `"Garbage Collection duration exceeds threshold: GC paused for 2100ms"` (quá trình GC bị throttle gây dừng lâu).
    *   `09:58:00Z` - `"Task execution rejected: java.util.concurrent.RejectedExecutionException: Task rejected... Queue capacity (500) exceeded."`.
    *   `10:00:00Z` - `"Thread pool starved. 50 tasks waiting... CPU throttle time: 1450ms"`.
    *   `10:01:00Z` - `"HTTP Request timeout on POST /v1/books: client aborted request after 15000ms"`.
...
*   **Nội dung**: Gom nhóm 1 alert nghiêm trọng xảy ra sau khi deploy:
    1.  `alert-book-service-5xx-spike` (started_at: `10:00:00Z` ngày `2026-06-29`): Cảnh báo tỷ lệ lỗi 5xx tăng đột biến trên `book-service`.

#### B. Chỉ số tài nguyên (Metrics Evidence)
*   **Đường dẫn**: `evidence/metrics/scenario_03_api_5xx_spike_metrics.json`
*   **Dữ liệu 5xx Rate (`http_5xx_rate`)**:
    *   Trước `09:58:00Z`: Bằng `0.0`.
    *   Lúc `10:00:00Z` - `10:02:00Z`: Bùng phát đạt đỉnh từ `15.2` tới `18.5` req/sec ngay sau khi deploy code mới.
...
*   Ghi nhận thông tin deployment mới gây lỗi:
    *   `version`: `v1.4.3` (deploy lúc `09:55:00Z`).
...
*   Mô tả lỗi logic code:
    *   `09:56:00Z` - `"Starting request handler on book-service version v1.4.3..."`.
    *   `10:00:01Z` - `"HTTP 500: Internal Server Error on POST /v1/books/promo: java.lang.NullPointerException at com.bookhub.promo.PromoHandler.applyPromoCode(PromoHandler.java:42)"`.

#### E. Sự kiện hệ thống (Kubernetes Events Evidence)
*   **Đường dẫn**: `evidence/k8s-events/scenario_03_api_5xx_spike_events.json`
*   *   `Readiness probe failed: HTTP probe failed with statuscode: 500` lúc `10:02:15Z`.
